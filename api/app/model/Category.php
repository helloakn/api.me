...
class Category extends Table{
    ...
    protected static $columnName=['id', 'name', 'image', 'description','parent_id'];
    ...
    function getCategory($parent_id=0){
        $condition = "parent_id=$parent_id AND deleted_at IS NULL";
        $rows = self::select("id,name,image,description,parent_id,created_at")->where($condition)->getAll();
        $data = [];
        if($rows){
            foreach($rows as $k=>$v){
                $v['child'] = $this->getCategory($v['id']);
                $data[] = $v;
            }
            return $data;
        }
    ...
    function _getList(){
        ...
        //return self::select("*")->paginate(2,1);
    }
